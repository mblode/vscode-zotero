...
Implemented and pushed as `ce75f8f53db8ca1a17bdf08a9fcc0a0488379601`. All 63 regressions and four cross-platform CI jobs pass. The extracted VSIX also passed local minimum/stable editor tests and live Zotero 10.0.1 / Better BibTeX 9.0.63 checks in an isolated profile. GitHub and Marketplace 0.2.0 are published; both public downloads exactly match the tested package. Open VSX 0.2.0 is also published, and its public download exactly matches the tested package. See `docs/compatibility-audit.md` for exact evidence, issue dispositions, and publication status.
